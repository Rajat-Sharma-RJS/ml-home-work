...
def batchGDA(features_matrix, output, alpha, epsilon, max_iter, reg = 0.0):
	converge = False
...
#setting
axis[0][0].plot(range(1000), (vec[len(vec)-1000:]/vec[len(vec)-1])*1000, '-')
axis[0][0].set_title("lambda = 0.0")
axis[0][0].set_xlabel("iteration")
axis[0][0].set_ylabel("Error")

print("With Regularization of 0.1")
weights, vec = batchGDA(features_matrix, output, alpha, epsilon, max_iter, 0.1)
print("weights :", weights)
...
#setting
axis[0][1].plot(range(1000), (vec[len(vec)-1000:]/vec[len(vec)-1])*1000, '-')
axis[0][1].set_title("lambda = 0.1")
axis[0][1].set_xlabel("iteration")
axis[0][1].set_ylabel("Error")

print("With Regularization of 0.01")
weights, vec = batchGDA(features_matrix, output, alpha, epsilon, max_iter, 0.01)
print("weights :", weights)
...
#setting
axis[1][0].plot(range(1000), (vec[len(vec)-1000:]/vec[len(vec)-1])*1000, '-')
...
axis[1][1].bar(x, ((y+5e4)-round(rss1)))
