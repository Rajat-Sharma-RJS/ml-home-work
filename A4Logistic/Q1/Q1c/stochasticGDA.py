...
def stochasticGDA(features_matrix, output, alpha, epsilon, max_iter, initial_weights, reg):
	converge = False
	...
	while not converge:
		...
		e = (abs(expected_value(features_matrix[k], weights) - output[k])).sum()
		arr.append(e)
		if abs(J-e) <= epsilon:
...
df['average_of2'] = (df['score1'] + df['score2'])/2
df['geometric_mean'] = (df['score1']*df['score2'])**(1/2)	# df[["A", "B"]].max(axis=1)
df['avg_lg'] = (df[['score1', 'score2']].min(axis=1) + df['geometric_mean'])/2
df['avg_ah'] = (df[['score1', 'score2']].max(axis=1) + df['average_of2'])/2

train, test = df[:round(len(df)*0.7)], df[round(len(df)*0.7):]
figure, axis = plt.subplots(1, 2)

#modeling data
features_matrix, output = get_np_data(train, ['score1', 'avg_lg', 'geometric_mean', 'average_of2', 'avg_ah', 'score2'], 'label')
test_features, test_output = get_np_data(test, ['score1', 'avg_lg', 'geometric_mean', 'average_of2', 'avg_ah', 'score2'], 'label')
...
weights, vec = stochasticGDA(features_matrix, output, alpha, epsilon, max_iter, initial_weights, reg)
...
axis[0].plot(range(1000), (vec[len(vec)-1000:]/vec[len(vec)-1]), '-')
...
weights_sc, vec_sc = stochasticGDA(features_matrix_sc, output, alpha, epsilon, max_iter, initial_weights_sc, reg)
...
axis[1].plot(range(1000), (vec_sc[len(vec_sc)-1000:]/vec_sc[len(vec_sc)-1]), '-')
