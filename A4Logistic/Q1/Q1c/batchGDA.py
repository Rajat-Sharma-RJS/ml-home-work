...
def batchGDA(features_matrix, output, alpha, epsilon, max_iter, initial_weights, reg):
	converge = False
...
df['geometric_mean'] = (df['score1']*df['score2'])**(1/2)
...
weights, vec = batchGDA(features_matrix, output, alpha, epsilon, max_iter, initial_weights, reg)
...
axis[0].plot(range(1000), (vec[len(vec)-1000:]/vec[len(vec)-1])*1e6, '-')
...
weights_sc, vec_sc = batchGDA(features_matrix_sc, output, alpha, epsilon, max_iter, initial_weights_sc, reg)
...
axis[1].plot(range(1000), (vec_sc[len(vec_sc)-1000:]/vec_sc[len(vec_sc)-1])*1e6, '-')
